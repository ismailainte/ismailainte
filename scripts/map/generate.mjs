...
fs.writeFileSync(
  path.join(APP, "mapAreas.ts"),
  "// Generated by scripts/map/generate.mjs - do not edit by hand.\n\n" +
    `export const REGION_AREAS_KM2: Record<string, number> = ${JSON.stringify(
      Object.fromEntries(
        regions.map((region) => [
          region.id,
          Math.round(geodesicMultiAreaKm2(regionShapes.get(region.id).multi)),
        ]),
      ),
    )};\n\n` +
    `export const DISTRICT_AREAS_KM2: Record<string, number> = ${JSON.stringify(
      Object.fromEntries(
        Object.values(districts)
          .flat()
          .map((district) => [
            district.id,
            Math.round(geodesicMultiAreaKm2(districtShapes.get(district.id).multi)),
          ]),
      ),
    )};\n`,
);
